"""Threshold filtering pipeline utilities.

This module wires `ScoresParser` into a Datatrove pipeline followed by a `JsonlWriter`.
It supports both local execution and Slurm execution via the respective Datatrove
executors.

The design mirrors `ml_filter.annotation.annotation_pipeline` and
`ml_filter.annotation.embedding_pipeline`.
"""
...
class ThresholdFilterPipelineBuilder(BaseSettings):
    ...
    ) -> "ThresholdFilterPipelineBuilder":
        if not path.is_file():
    ...
    def build_pipeline(self) -> list[PipelineStep]:
        ...
        reader: PipelineStep = PairedThresholdFilter(
...
def run_threshold_filter_pipeline(config: Path | ThresholdFilterPipelineBuilder):
    """Run the threshold filter pipeline.

    Args:
        config: Either a Path to a YAML config file (builder-style schema) or an
            already constructed ThresholdFilterPipelineBuilder.
    """
    builder = config if isinstance(config, ThresholdFilterPipelineBuilder) else ThresholdFilterPipelineBuilder.from_yaml(config)
...
def _main(argv: list[str] | None = None) -> int:
    ...
    Usage:
        python -m ml_filter.data_processing.jsonl_filtering.threshold_filter_pipeline <config.yaml>
    """
    import argparse

    parser = argparse.ArgumentParser(description="Run threshold filter pipeline from a YAML config")
    parser.add_argument("config", type=Path, help="Path to a builder-style YAML config")
    args = parser.parse_args(argv)

    run_threshold_filter_pipeline(args.config)
    return 0
