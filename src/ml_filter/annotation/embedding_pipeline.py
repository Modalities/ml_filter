...
class EmbeddingPipelineBuilder(BaseSettings):
    ...
    def from_yaml(
        ...
        if not rs and isinstance(local_section, dict):
            allowed_keys = {"tasks", "workers", "local_tasks", "local_rank_offset", "logging_dir"}
            local_kwargs = {k: local_section[k] for k in allowed_keys if k in local_section}
            local_settings_obj = LocalExecutionSettings(**local_kwargs)
