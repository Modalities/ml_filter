...
class JQLJsonlReader(BaseDiskReader):
    ...
    Args:
        ...
        glob_pattern: pattern that all files must match exactly to be included
        (relative to data_folder). Ignored if paths_file is provided
        shuffle_files: shuffle the files within the returned shard.
        Mostly used for data viz. purposes, do not use with dedup blocks
    """
    ...
    def __init__(
        ...
        # Only text_key and id_key is read into as text, everything else is metadata
        text_key: str = "text",
        id_key: str = "id",
                    ...
                    try:
                        ...
                        for idx, (doc, embedding) in enumerate(zip(doc_batch, embeddings)):
                            ...
                            if doc.metadata["score"]:
                                # Since everything else is metadata, we now fetch the score from metadata
                                doc.metadata["score"] = doc.metadata["score"]
                            if writer:
                        ...
                        logger.info(
                            f"""Processed {num_docs} docs in {duration:.2f}s in rank {rank} → Throughput: 
...
class HDF5Writer(DiskWriter):
    ...
    def _write_batch(self, filename: str):
        ...
        labels = np.array([doc["metadata"]["score"] for doc in batch], dtype=np.float32)
        ...
        if group_name not in file:
            ...
            maxshape_ids = (None,)
            maxshape_labels = (None, 5)  # Labels can be from 0-5 tasks
            ...
            group.create_dataset("labels", data=labels, maxshape=maxshape_labels, compression="gzip", dtype=np.float32)
        else:
