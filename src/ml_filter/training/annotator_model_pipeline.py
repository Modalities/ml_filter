...
def extract_and_save_embeddings(config_file_path: Path, output_dir: Path):
    ...
    train_embeddings, train_labels = _extract_embeddings_from_dataset(
        model=model, dataset=train_dataset, tokenizer=tokenizer, device=device, batch_size=cfg.training.batch_size
    )
...
def _extract_embeddings_from_dataset(model, dataset, tokenizer, device, batch_size=4):
